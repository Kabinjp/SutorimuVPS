...
if [ "$arch" = "x86_64" ]; then
	if [ "$OS_MAJOR_VERSION" = 6 ]; then
		yum -y --enablerepo=remi -y install mysql mysql-server;
		yum -y --enablerepo=remi -y install make gcc gcc-c++ patch gcc-g77 flex bison tar unzip ntp pcre perl pcre-devel zlib zlib-devel GeoIP GeoIP-devel openssl openssl openssl-devel;
	else 
